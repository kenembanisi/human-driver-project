...
function [pot_x, pot_y, pot_z, pot_t] = PotField3D_pedals(AuxData)
...
ax = zeros(1,AuxData.NumTimSteps);
ay = zeros(1,AuxData.NumTimSteps);
az = zeros(1,AuxData.NumTimSteps);
at = zeros(1,AuxData.NumTimSteps);

ax(1,1) = x_ar(1)/qscale + qcenter(1);
ay(1,1) = y_ar(1)/qscale + qcenter(2);
az(1,1) = z_ar(1)/qscale + qcenter(3);
at(1,1) = t_ar(1);

ax(1,end) = x_ar(end)/qscale + qcenter(1);
ay(1,end) = y_ar(end)/qscale + qcenter(2);
az(1,end) = z_ar(end)/qscale + qcenter(3);
at(1,end) = t_ar(end);

trf_ar = t_ar(length(t_ar));

for i = 2:(AuxData.NumTimSteps-1)
    ax(1,i) = interp1(t_ar,x_ar,trf_ar/(AuxData.NumTimSteps-1)*(i-1))/qscale + qcenter(1);
    ay(1,i) = interp1(t_ar,y_ar,trf_ar/(AuxData.NumTimSteps-1)*(i-1))/qscale + qcenter(2);
    az(1,i) = interp1(t_ar,z_ar,trf_ar/(AuxData.NumTimSteps-1)*(i-1))/qscale + qcenter(3);
    at(1,i) = interp1(t_ar,t_ar,trf_ar/(AuxData.NumTimSteps-1)*(i-1));
end
